...
def create_env(config: dict[str, Any]):
    ...
    exchange = Exchange(
        "exchange", service=execute_order, options=ExchangeOptions(commission=0.0005)
...
def main():
    ...
    print(
        f"Train period: {train_data['date'].iloc[0].date()} to {train_data['date'].iloc[-1].date()}"
    )
    print(
        f"Test period: {test_data['date'].iloc[0].date()} to {test_data['date'].iloc[-1].date()}"
    )
    ...
    for i in range(100):
        ...
        if (i + 1) % 10 == 0:
            ...
            print(
                f"Iter {i + 1:3d} | Train: {reward:+8,.0f} P&L ${pnl:+6,.0f} | "
