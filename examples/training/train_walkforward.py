...
def main():
    ...
    print(
        f"Market regimes: Bull {bull_pct:.1f}% | Bear {bear_pct:.1f}% | Sideways {sideways_pct:.1f}%"
    )

    feature_cols = [
        col
        for col in data.columns
        if col not in ["date", "regime"] and data[col].dtype in ["float64", "int64"]
    ]
    ...
    if len(bull_data) > fold_hours:
        ...
        bull_segments = []
        start_idx = None
        for i, (idx, _row) in enumerate(bull_data.iterrows()):
        ...
        if bull_segments:
            ...
            folds.append(
                (
                    "Bull Market",
                    train_pool.loc[best_seg[0] : best_seg[1]].head(fold_hours).copy(),
                )
            )
    ...
    if len(bear_data) > fold_hours // 2:
        bear_segments = []
        start_idx = None
        for i, (idx, _row) in enumerate(bear_data.iterrows()):
            if start_idx is None:
                start_idx = idx
    ...
    print("-" * 80)
    print(
        f"{'Iter':>4} | {'Avg Reward':>12} | {'Net Worth':>12} | {'P&L':>12} | Status"
    )
    print("-" * 80)
    ...
    for i in range(total_iterations):
        ...
        if ep_reward is not None:
            ...
            if (i + 1) % 5 == 0 or marker or i < 5:
                ...
                print(
                    f"{i + 1:4d} | {ep_reward:>+12,.0f} | {nw_str:>12} | {pnl_str:>12} | {marker}"
                )
    ...
    print(
        f"Price range: ${eval_data['close'].min():,.0f} - ${eval_data['close'].max():,.0f}"
    )
    print("Running 30 evaluation episodes...\n")
    ...
    print(
        f"  Avg Final Worth:   ${np.mean([r['final_worth'] for r in eval_results['results']]):,.2f}"
    )
    print(
        f"  Avg P&L:           ${eval_results['avg_pnl']:+,.2f} ({eval_results['avg_pnl_pct']:+.2f}%)"
    )
    ...
    if diff > 0:
        ...
        print(
            f"  (Agent showed {eval_results['avg_pnl_pct']:+.2f}% vs B&H {bh_return:+.2f}%)"
        )
