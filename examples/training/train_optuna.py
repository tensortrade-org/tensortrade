...
def evaluate(
    algo, data: pd.DataFrame, feature_cols: list, config: dict, n: int = 10
) -> float:
...
def main():
    ...
    TRAIN_DATA = (
        data.iloc[: -(test_candles + val_candles)].tail(3000).reset_index(drop=True)
    )
    ...
    print("Each trial: 40 training iterations, evaluated on validation set\n")

    def callback(study, trial):
        if trial.state == optuna.trial.TrialState.COMPLETE:
            print(
                f"Trial {trial.number:2d}: Val P&L ${trial.value:+,.0f} | "
    ...
    study.optimize(
        objective, n_trials=n_trials, callbacks=[callback], show_progress_bar=False
    )
    ...
    final_config = (
        ...
        .training(
            ...
            vf_clip_param=100.0,
            model={
                "fcnet_hiddens": [best["hidden_size"], best["hidden_size"]],
                "fcnet_activation": "tanh",
            },
        )
        .resources(num_gpus=0)
    ...
    for i, (_, row) in enumerate(trials_df.head(5).iterrows()):
        print(
            f"{i + 1}. Trial {int(row['number'])}: Val P&L ${row['value']:+,.0f} | "
