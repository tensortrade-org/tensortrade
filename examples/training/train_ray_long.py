...
def main():
    ...
    print(
        f"Market move: ${price_start:,.0f} -> ${price_end:,.0f} ({price_change_pct:+.1f}%)"
    )
    ...
    print("  Reward Scheme:    PBR (Position-Based Returns)")
    print("  Action Scheme:    BSH (Buy/Sell/Hold)")
    print("  Workers:          4 parallel environments")
    print("  Batch Size:       8000 steps")
    print("  Target Duration:  5-10 minutes")
    ...
    print("-" * 90)
    print(
        f"{'Iter':>4} | {'Episodes':>8} | {'Avg Reward':>12} | {'Avg Net Worth':>14} | {'Avg P&L':>12} | Progress"
    )
    print("-" * 90)
    ...
    for i in range(100):
        ...
        if ep_reward_mean is not None:
            ...
            print(
                f"{i + 1:4d} | {episodes_this_iter:>8} | {ep_reward_mean:>+12,.0f} | {net_worth_str:>14} | {pnl_str:>12} | {bar}{marker}"
            )
        else:
