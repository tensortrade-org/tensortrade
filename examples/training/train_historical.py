    ...
    def on_episode_end(
        ...
        **kwargs: dict[str, Any],
        ...
        if hasattr(env, "portfolio"):
            ...
            episode.custom_metrics["pnl_pct"] = (
                (pnl / initial_worth) * 100 if initial_worth > 0 else 0
            )
...
def main():
    ...
    print(
        f"  Price range: ${train_data['close'].min():,.0f} - ${train_data['close'].max():,.0f}"
    )
    ...
    print("-" * 80)
    print(
        f"{'Iter':>4} | {'Avg Reward':>12} | {'Avg Net Worth':>14} | {'Avg P&L':>12} | {'Status':>12}"
    )
    print("-" * 80)
    ...
    for i in range(num_iterations):
        ...
        if ep_reward_mean is not None:
            ...
            net_worth_str = (
                f"${final_net_worth_mean:,.0f}" if final_net_worth_mean else "N/A"
            )
            ...
            if (i + 1) % 5 == 0 or marker or i < 5:
                print(
                    f"{i + 1:4d} | {ep_reward_mean:>+12,.0f} | {net_worth_str:>14} | {pnl_str:>12} | {marker:>12}"
                )
    ...
    if len(rewards_history) >= 10:
        ...
        print(
            f"  Improvement: {improvement:+,.0f} ({improvement / abs(first_10) * 100 if first_10 != 0 else 0:+.1f}%)"
        )
    ...
    for ep in range(num_eval_episodes):
        ...
        print(
            f"Episode {ep + 1:2d} | Steps: {steps:4d} | Reward: {total_reward:>+10,.0f} | "
    ...
    print(
        f"Evaluation Period: {eval_data['date'].iloc[0].date()} to {eval_data['date'].iloc[-1].date()}"
    )
    print(f"\nEvaluation Results ({num_eval_episodes} episodes on recent prices):")
    print("  Initial Capital:  $10,000")
    print(
        f"  Avg Final Worth:  ${np.mean([r['final_worth'] for r in eval_results]):,.2f}"
    )
