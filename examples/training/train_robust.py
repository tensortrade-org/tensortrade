...
def create_env(config: dict[str, Any]):
    ...
    exchange = Exchange(
        "exchange", service=execute_order, options=ExchangeOptions(commission=0.001)
    )(price)
...
def evaluate_on_data(
    algo, data: pd.DataFrame, feature_cols: list, num_episodes: int = 10
) -> dict:
...
def main():
    ...
    print(
        f"Total: {len(data):,} candles ({data['date'].min().date()} to {data['date'].max().date()})"
    )
    ...
    print("\nData splits:")
    print(
        f"  Train: {len(train_data):,} candles ({train_data['date'].iloc[0].date()} to {train_data['date'].iloc[-1].date()})"
    )
    print(
        f"  Val:   {len(val_data):,} candles ({val_data['date'].iloc[0].date()} to {val_data['date'].iloc[-1].date()})"
    )
    print(
        f"  Test:  {len(test_data):,} candles ({test_data['date'].iloc[0].date()} to {test_data['date'].iloc[-1].date()})"
    )
    ...
    print("  Network: [64, 64] (small)")
    print("  Entropy: 0.05 (high exploration)")
    print("  Clip: 0.1 (conservative updates)")
    print()
    ...
    print("-" * 90)
    print(
        f"{'Iter':>4} | {'Train Reward':>12} | {'Train P&L':>10} | {'Val P&L':>10} | {'Val vs B&H':>10} | Status"
    )
    print("-" * 90)
    ...
    for i in range(max_iterations):
        ...
        train_pnl = (
            result.get("env_runners", {}).get("custom_metrics", {}).get("pnl_mean", 0)
        )
        ...
        if (i + 1) % 10 == 0:
            ...
            print(
                f"{i + 1:4d} | {train_reward:>+12,.0f} | ${train_pnl:>+8,.0f} | ${val_pnl:>+8,.0f} | ${vs_bh:>+8,.0f} | {status}"
            )

            # Early stopping
            if (
                no_improve_count >= patience // 10
            ):  # Check every 10 iters, so patience/10
                print(
            ...
            if (i + 1) % 5 == 0 or i < 5:
                print(
                    f"{i + 1:4d} | {train_reward:>+12,.0f} | ${train_pnl:>+8,.0f} |            |            |"
                )
    ...
    print(
        f"  Avg P&L:     ${test_result['avg_pnl']:+,.2f} ({test_result['avg_pnl_pct']:+.2f}%)"
    )
    ...
    if diff > 0:
        ...
        if test_bh_pnl < 0:
            print(
                f"  (Agent lost {test_result['avg_pnl_pct']:.1f}% vs B&H lost {test_bh_pct:.1f}%)"
            )
        else:
            print(
                f"  (Agent: {test_result['avg_pnl_pct']:+.1f}% vs B&H: {test_bh_pct:+.1f}%)"
            )
