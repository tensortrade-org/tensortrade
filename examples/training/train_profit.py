...
def find_bear_periods(
    df: pd.DataFrame, window: int = 720, threshold: float = -0.02
) -> list:
...
def evaluate(
    algo, data: pd.DataFrame, feature_cols: list, config: dict, n: int = 20
) -> float:
...
def main():
    ...
    if bear_periods:
        ...
        for start, end, ret in selected:
            ...
            print(
                f"  Period {remaining_data['date'].iloc[start].date()} to "
    ...
    for i in range(train_iters):
        ...
        if (i + 1) % 5 == 0:
            ...
            print(
                f"  Iter {i + 1:3d}: Train P&L ${pnl:+,.0f} | Val P&L ${val_pnl:+,.0f} "
