...
.PHONY: clean install install-test install-lint install-docs install-dev install-all sync lock test test-parallel doctest docs-build docs-clean docs-serve lint format typecheck dev stop dev-status init-db reset-db build-cpu build-gpu build-cpu-if-not-built build-gpu-if-not-built run-notebook run-docs run-tests run-notebook-gpu run-docs-gpu run-tests-gpu package test-release release
