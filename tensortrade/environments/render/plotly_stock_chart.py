...
class PlotlyTradingChart:
    ...
    def _create_figure(self, performance_keys):
            fig = make_subplots(
                ...
                subplot_titles = ['Price', 'Volume', 'Performance', 'Net Worth']
            )
            fig.add_trace(go.Candlestick(name='Price', xaxis='x1', yaxis='y1'), row=1, col=1)

            fig.add_trace(
                go.Bar(name='Volume', marker={ 'color': 'DodgerBlue' }),
                row=2, col=1
                )
            ...
            fig.update_layout(height=self._height, xaxis=dict(autorange=False, range=[0, 100], type='linear'))
            ...
            for i in self.fig['layout']['annotations']:  # hacky modification of subplot titles
                i['font'] = dict(size=12)
    ...
    def save(self, filename, directory='charts', fmt='png'):
        """
        ...
        :param filename: str, primary part of the image file name (e.g. "mountain" in "mountain.png".
        :param directory: str, directory to save to.
        :param fmt: str, the image format to save.
        """
