...
class TradingEnvironment(gym.Env):
    ...
    def __init__(self,
        ...
        self._observation_space_lows = kwargs.get('observation_space_lows', 0)
        self._observation_space_highs = kwargs.get('observation_space_highs', 1e8)
    ...
    def observation_space(self) -> Box:
        ...
        if isinstance(self._observation_space_lows, list) and len(self._observation_space_lows) != n_features:
            raise ValueError(
                'The length of `observation_space_lows` provided to the exchange must match the length of `observation_columns`.')

        if isinstance(self._observation_space_highs, list) and len(self._observation_space_highs) != n_features:
            raise ValueError(
                'The length of `observation_space_highs` provided to the exchange must match the length of `observation_columns`.')

        low = self._observation_space_lows if isinstance(
            self._observation_space_lows, list) else np.tile(self._observation_space_lows, n_features)
        high = self._observation_space_highs if isinstance(
            self._observation_space_highs, list) else np.tile(self._observation_space_highs, n_features)
