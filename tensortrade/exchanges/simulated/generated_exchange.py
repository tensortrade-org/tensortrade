...
class GeneratedExchange(AssetExchange):
    ...
    def reset(self):
        ...
        self._net_worth = self._initial_balance
    ...
    def net_worth(self, output_symbol: str = 'USD') -> float:
        net_worth = self._balance

        for symbol, amount in vars(self.portfolio).items():
    ...
    def profit_loss_percent(self) -> float:
        return float(self._net_worth / self._initial_balance)
    ...
    def next_observation(self):
        dates = np.linspace(0, self.current_index, self.current_index + 1)
