...
from .agents.tests.utils import assert_flags_displayed, get_expected_flags
from .agents.utils.common import create_model
...
class TestExecutor:
    ...
    def assert_base_displayed(cap):
        ...
        for keyword in [
            'tensortrade',
            'Usage',
            'tensortrade <command> <agent> [options] [args]',
            'Available commands:',
        ]:
    ...
    def test_maybe_create_agent_command_agent(self, command, agent_id, capsys):
        ...
        Args:
            command: One of the commands available in tensortrade.agents.commands
