...
from tensortrade.agents.tests.utils import assert_flags_displayed, get_expected_flags
from tensortrade.agents.utils.common import create_model
...
class TestExecutor:
    ...
    def assert_base_displayed(cap):
        ...
        for keyword in [
            'agents',
            'Usage',
            'agents <command> <agent> [options] [args]',
            'Available commands:',
        ]:
    ...
    def test_maybe_create_agent_command_agent(self, command, agent_id, capsys):
        ...
        Args:
            command: One of the commands available in agents.commands
            agent_id: One of the agent ids available in agents.agents
            capsys: _pytest.capture.CaptureFixture
        """
    ...
    def test_create_models(self, agent_id):
        ...
        for model_type in ['model', 'actor_model', 'critic_model']:
            if model_type in agents.agents[agent_id]:
