...
from .agents.utils.buffers import ReplayBuffer1, ReplayBuffer2
...
def register_models(agents):
    ...
    configuration folders to be added to tensortrade.agents.agents.
    Args:
        agents: tensortrade.agents.agents
...
def create_model(
    ...
    Args:
        env: gym environment.
        agent_id: str, one of the keys in tensortrade.agents.agents
    ...
    if len(env.observation_space.shape) == 3:
        ...
        network_type = 'ann'
    try:
        model_cfg = model_cfg or agents.agents[agent_id][model_type][network_type][0]
    except IndexError:
        model_cfg = None
    models_folder = Path(agents.agents[agent_id]['module'].__file__).parent / 'models'
    assert model_cfg, (
...
def create_models(options, env, agent_id, **kwargs):
    ...
    Args:
        ...
        env: gym environment
        agent_id: str, one of the keys in tensortrade.agents.agents
        **kwargs: kwargs passed to tensortrade.agents.utils.common.create_model()
...
def create_agent(agent_id, agent_kwargs, non_agent_kwargs, trial=None):
    ...
    if (
        issubclass(agents.agents[agent_id]['agent'], agents.OffPolicy)
        or agent_id == 'acer'
    ):
    ...
    agent = agents.agents[agent_id]['agent'](**agent_kwargs)
