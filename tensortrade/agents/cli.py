...
from tensortrade.agents.base import OffPolicy
from tensortrade.agents.utils.cli import agent_args, non_agent_args, off_policy_args
from tensortrade.agents.utils.common import create_agent
from tensortrade.agents.utils.tuning import run_tuning
...
class Executor:
    ...
    def display_commands(self, sections=None):
        ...
        print(f'\nUsage:')
        print(f'\tagents <command> <agent> [options] [args]')
        print(f'\nAvailable commands:')
        for command, items in agents.commands.items():
            print(f'\t{command:<10} {items[2]}')
        print()
        print('Use agents <command> to see more info about a command')
        print('Use agents <command> <agent> to see more info about command + agent')
        if sections:
    ...
    def maybe_create_agent(self, argv):
        ...
        assert command in agents.commands, f'Invalid command `{command}`'
        to_display.update(agents.commands[command][0])
        if total == 1:
            self.display_commands({command: to_display})
            return
        agent_id = argv[1]
        assert agent_id in agents.agents, f'Invalid agent `{agent_id}`'
        to_display.update(agents.agents[agent_id]['module'].cli_args)
        if total == 2:
            title = f'{command} {agent_id}'
            if (
                issubclass(agents.agents[agent_id]['agent'], OffPolicy)
                or agent_id == 'acer'
            ):
    ...
    def parse_known_args(self, argv, tuning=False):
        ...
        self.add_args(
            agents.agents[self.agent_id]['module'].cli_args, agent_parser, tuning
        )
        self.add_args(agents.commands[self.command][0], command_parser, tuning)
        if (
            issubclass(agents.agents[self.agent_id]['agent'], OffPolicy)
    ...
    def execute(self, argv):
        ...
        if self.command == 'tune':
            ...
            getattr(self.agent, agents.commands[self.command][1])(
                **vars(command_known)
            )
