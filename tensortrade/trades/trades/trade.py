...
class Trade(object):
    ...
    def __init__(self, symbol: str, trade_type: 'TradeType', amount: float, price: float, step: int):
        """
        Arguments:
            ...
            step: The step at which the trade was taken (this is tracked here to ensure the incorrect step is not sused to retreive info later)
        """
        self._symbol = symbol
    ...
    def copy(self) -> 'Trade':
        ...
        return Trade(symbol=self._symbol, trade_type=self._trade_type, amount=self._amount, price=self._price, step=self._step)
    ...
    def step(self) -> int:
        """
        Returns:
            int -- the step at which the trade was made 
        """
        return self._step